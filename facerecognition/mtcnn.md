...
$$
分子为预测结果和ground truth关键点之间的距离,分母为ground truth两眼中心的间距,目的是对距离做一个归一化屏蔽脸部大小问题.
...
（2）输入：训练输入尺寸大小为 12x12的三通道图像 ,这里注意是将原图随机裁剪成12*12.测试时才输入的图像金字塔。

（3）输出：包含三部分：a.是否人脸的概率1x1x2向量；b.人脸检测框坐标（左上点和右下点）1x1x4向量；c.人脸关键点（5个关键点）坐标1x1x10向量。(后面三部分是由1*1卷积改变通道数得到的)
...
由于在MTCNN 中，每个网络都有三个输出，因此会对应有三个损                                                                                                                                                                                         失函数：
